...
data = data.dropna(subset=features + ["Price/Earnings"]).copy()

# Target (already sign log-transformed in the data)
target = data["Price/Earnings"]
...
for i,j in pairs:
    ...
    plt.colorbar(scatter, label='sign_log(P/E)')
    plt.xlabel(f"PC{i+1}")
    plt.ylabel(f"PC{j+1}")
...
for i,j,k in triples:
    ...
    fig.colorbar(p, ax=ax, label="sign_log(P/E)")
    plt.tight_layout()
